...
@objc public class SwrveSDK: NSObject {
    ...
    /// The default user ID is a random UUID. The userID is cached in the default settings of the app and recalled the next time you initialize the
    /// app. This means the ID for the user will stay consistent for as long as the user has your app installed on the device.
    /// - Parameters:
    ///   - withAppID: The App ID for your app supplied by Swrve.
    ///   - apiKey: The secret token for your app supplied by Swrve.
    ...
    ///   - withAppID: The App ID for your app supplied by Swrve.
    ///   - apiKey: The secret token for your app supplied by Swrve.
    ///   - config: The swrve configuration object used to override default settings.
    ...
    /// The currency specified must be one of the currencies known to Swrve that are specified on the Swrve dashboard.
    /// - Parameters:
    ...
    /// See the REST API docs for the IAP event for a detailed description of the semantics of this call, noting in particular the format specification for currency.
    /// - Parameters:
    ...
    /// Call this when the user has bought something using real currency.
    /// Include the virtual item and currency given to the user in rewards. See the REST API docs for the IAP event for a detailed description of the
    /// semantics of this call, noting in particular the format specification for currency.
    /// - Parameters:
    ///   - rewards: The SwrveIAPRewards object containing any additional items or in-app currencies that are part of this purchase.
    ...
    /// - Parameters:
    ///   - rewards: The SwrveIAPRewards object containing any additional items or in-app currencies that are part of this purchase.
    ///   - localCost: The price (in real money) of the product that was purchased. Note: this is not the price of the total transaction, but the per-product price.
    ...
    @objc public class func unvalidatedIap(
        ...
        checkInstance()
        let result = sharedInstance.unvalidatedIap(
            rewards,
            localCost: localCost,
            localCurrency: localCurrency,
            productId: productId,
            productIdQuantity: Int32(productIdQuantity)
        )
        return Int(result)
    }
    ...
    /// See the REST API docs for the currency_given event for a detailed description of the semantics of this call.
    /// - Parameters:
    ///   - given: The name of the in-app currency that the player was rewarded with.
    ...
    /// See the REST API docs for the user event for a detailed description of the semantics of this call.
    ...
    ///   - with: The NSDate value associated.
    /// - Returns: SWRVE_SUCCESS if the call was successful, otherwise SWRVE_FAILURE.
    ...
    /// If SwrveConfig.autoDownloadCampaignsAndResources is YES (default value) this function is called automatically to keep the user resources and
    /// campaign data up to date. Use the resourceManager to get the latest up-to-date values for the resources.
    ...
    /// Gets a list of resources for a user including modifications from active A/B tests.  Please refer to our online documentation for more details:
    /// http://dashboard.swrve.com/help/docs/abtest_api#GetUserResources
    /// This function will return immediately, and the callback will be fired right away with the already present AB test data.
    /// The result of this call is cached in the userResourcesCacheFile specified in SwrveConfig. This file is initially seeded with "[]", the empty JSON array.
    /// - Parameter callbackBlock: A callback block that will be called asynchronously when A/B test data is available.
    ...
    /// Gets the user resource differences that should be applied to items for the given user based on the A/B test the user is involved in.  Please refer to
    ...
    /// This function issues an asynchronous HTTP request to the Swrve content server specified in #swrve_init. This function will return immediately, and the
    /// callback may be fired at some unspecified time in the future. The callback will be fired after the Swrve server has sent some AB-Test modifications to
    /// the SDK, or if the HTTP request fails (when the iOS device is offline or has limited connectivity.
    ...
    /// This function will return immediately, and the callback will be fired right away with the already present AB test data.
    /// - Parameter callbackBlock: A callback block that will be called asynchronously when real time user properties become available
    ...
    /// Sends all events that are queued to the Swrve servers. If any events cannot be send they will be re-queued and sent again later.
    @objc public class func sendQueuedEvents() {
        checkInstance()
        sharedInstance.sendQueuedEvents()
    }

    /// Saves events stored in the in-memory queue to disk. After calling this function, the in-memory queue will be empty.
    @objc public class func saveEventsToDisk() {
        checkInstance()
        sharedInstance.saveEventsToDisk()
    }

    /// Sets the event queue callback. If set, the callback block will be called each time an event is queued with the SDK.
    ...
    /// Similar to `event(_:payload:)` except the callback block will not be called when the event is queued.
    /// - Parameters:
    ...
    /// Releases all resources used by the Swrve object. Typically, this should only be called if you are managing multiple Swrve instances.
    ...
    /// - Parameters:
    ///   - externalUserId:An ID that uniquely identifies your user. Personal identifiable information should not be used. An error may be returned if such information is
    ///   submitted as the userID eg email, phone number etc.
    ...
    ///         // the tracked userId will not reflect correctly on the backend until this call completes successfully
    /// })
    /// ```
    ...
    /// An ID that uniquely identifies your user. Personal identifiable information should not be used. See the identify API call.
    ...
    /// Start the SDK when in SwrveInitMode (Managed) mode. Tracking will begin using the userId passed in.
    /// Can be called multiple times to switch the current userId to something else. A new session is started if not already
    /// started or if is already started with different userId. The SDK will remain started until the createInstance is called again.
    ...
    /// Inform that an embedded message has been served and processed. This function should be called by your implementation to update the
    /// campaign information and send the appropriate data to Swrve.
    ...
    @objc public class func personalizeEmbeddedMessageData(
        _ message: SwrveEmbeddedMessage,
        withPersonalization personalizationProperties: [String: Any]
    ) -> String? {
        checkInstance()
    ...
    /// that have been deleted with the removeCampaign method and those that do not support the current orientation.
    /// - Parameters:
